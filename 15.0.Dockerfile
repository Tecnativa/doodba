...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    # We install psycopg2 from via apt because it randomly fails to build when installed
    # from pip in CI, and its the same version: 2.8.6
    && apt-get install -yqq --no-install-recommends python3-psycopg2\
